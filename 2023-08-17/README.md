...
```
4. **로컬 브랜치 목록 보기**: 현재 로컬 저장소에 있는 모든 브랜치 목록을 확인한다.
```bash
git branch
```
5. **원격 브랜치 목록 보기**: 원격 저장소에 있는 브랜치 목록을 확인한다.
```bash
git branch -r
```
6. **브랜치 삭제하기**: 브랜치를 삭제한다. (-d 옵션은 안전하게 삭제하고, -D 옵션은 강제로 삭제한다.)
```bash
git branch -d <branch_name>
```
7. **원격 브랜치 가져오기**: 원격 저장소의 최신 브랜치 정보를 가져온다.
```bash
git fetch
```
8. **브랜치 병합하기**: 다른 브랜치를 현재 브랜치로 병합한다.
```bash
git merge <branch_name>
```
9. **브랜치 리베이스**: 현재 브랜치에 다른 브랜치의 변경사항을 리베이스한다.
```bash
git rebase <branch_name>
```
10. **현재 브랜치 정보 확인하기**: 현재 작업 중인 브랜치를 확인한다.
```bash
