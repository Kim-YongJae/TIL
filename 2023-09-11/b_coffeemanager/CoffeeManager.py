...
def main():
    ...
    # 지금 6,7,8월이 아니라면 프라푸치노를 주문할 경우 '시즌 상품은 비시즌에 구매할 수 없습니다'를 출력하고 주문취소
    # from datetime import datetime
    # now_month = datetime.now().month
    # 만약 시즌 음료가 11종류이고 11종류마다 시즌월이 다 다르다면 어떻게 확장할까
    Frappuccino = Coffee("프라푸치노", 5000, 4000, 10, 3, 0)
    drinks = [americano, moca, latte, Frappuccino]
    ...
    while True:
        ...
        if input_menu == 1:
            ...
            if drinks[input_code].price * order_cnt >= 10000:
                drinks[input_code].stock -= order_cnt
                balance += drinks[input_code].price * 0.9 * order_cnt
                sales_price += drinks[input_code].price * 0.9 * order_cnt
            ...
            if drinks[input_code].price * order_cnt >= 10000: 
                print("\n 제품명 :", drinks[input_code].name)
                print(" 판매가 :", drinks[input_code].price * 0.9)
                print(" 판매수량 :", order_cnt)
                print(" 결제금액 :", drinks[input_code].price * 0.9 * order_cnt)
